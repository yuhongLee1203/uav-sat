...
def render_video(
    ...
    try:
        for row_index, row in rows.iterrows():
            ...
            # Deliberately NO heading/body arrow. This architecture does not
            # estimate UAV yaw; Kalman velocity is movement direction, not
            # aircraft body orientation.

            lines = [
                "GT = GREEN    FINAL PRED = MAGENTA",
                "frame=%d" % int(row["frame_id"]),
                "Poly-Pred=(%.1f, %.1f)  CRF-HardMS=(%.1f, %.1f)"
                % (
                    float(row["predicted_current_x"]),
                    float(row["predicted_current_y"]),
                    float(row["hardms_x"]),
                    float(row["hardms_y"]),
                ),
                "RNN stateNorm=%.3f motion=%.2fm gate=%.3f"
                % (
                    float(row["state_norm"]),
                    float(row["model_next_step_m"]),
                    float(row["correction_gate"]),
                ),
                "KF speed=%.2fm/frame  error=%.2fm"
                % (
...
def render_route(route_name):
    csv_path = (
        ...
        / (
            route_name
            + "_hardms_state_rnn_kalman_frames.csv"
        )
    )
