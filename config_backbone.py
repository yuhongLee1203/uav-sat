import os
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parent

# ============================================================================
# Backbone ablation for the T2-only RTL-CRF
# ============================================================================
# Examples:
#   RTL_BACKBONE=vgg16
#   RTL_BACKBONE=resnet18
#   RTL_BACKBONE=mobilenet_v3_small
#   RTL_BACKBONE=resnet50
#   RTL_BACKBONE=mobileclip2_s2
#
# The default temporal model is the current final choice:
# 5-frame, T2-only RTL-CRF.
# ============================================================================
BACKBONE_KEY = os.environ.get("RTL_BACKBONE", "vgg16").strip().lower()

_BACKBONE_SPECS = {
    "mobileclip2_s2": {
        "name": "hf-hub:timm/MobileCLIP2-S2-OpenCLIP",
        "feature_dim": 512,
    },
    "vgg16": {
        "name": "torchvision:VGG16_IMAGENET1K_V1",
        "feature_dim": 512,
    },
    "resnet18": {
        "name": "torchvision:ResNet18_IMAGENET1K_V1",
        "feature_dim": 512,
    },
    "mobilenet_v3_small": {
        "name": "torchvision:MobileNetV3-Small_IMAGENET1K_V1",
        "feature_dim": 576,
    },
    "resnet50": {
        "name": "torchvision:ResNet50_IMAGENET1K_V2",
        "feature_dim": 2048,
    },
}

if BACKBONE_KEY not in _BACKBONE_SPECS:
    raise ValueError(
        "RTL_BACKBONE must be one of "
        f"{sorted(_BACKBONE_SPECS)}, got {BACKBONE_KEY!r}"
    )

BACKBONE_NAME = _BACKBONE_SPECS[BACKBONE_KEY]["name"]
CLIP_DIM = int(_BACKBONE_SPECS[BACKBONE_KEY]["feature_dim"])

TEMPORAL_WINDOW = int(os.environ.get("RTL_TEMPORAL_WINDOW", "5"))
if TEMPORAL_WINDOW not in (3, 4, 5):
    raise ValueError(
        "RTL_TEMPORAL_WINDOW must be one of {3,4,5}; "
        f"got {TEMPORAL_WINDOW}"
    )

OUTPUT_DIR = (
    PROJECT_ROOT
    / "outputs"
    / f"backbone_ablation_{BACKBONE_KEY}_t2only_w{TEMPORAL_WINDOW}"
)
CHECKPOINT_DIR = OUTPUT_DIR / "checkpoints"
VISUAL_CHECKPOINT = CHECKPOINT_DIR / "visual_retrieval_A_only.pt"
TEMPORAL_CHECKPOINT = CHECKPOINT_DIR / "rtl_crf_A_only.pt"

DATASETS_ROOT = Path("/yh/study/new_data_2")
ROUTE_ROOTS = [
    DATASETS_ROOT / "model_dataset_new_1_flight",
    DATASETS_ROOT / "model_dataset_new_2_flight",
    Path("/yh/study/new_data/model_dataset_flight"),
]
ROUTE_NAMES = ["route_A", "route_B", "route_C"]
TRAIN_ROUTE_NAMES = ["route_A"]
EVAL_ROUTE_NAMES = ["route_B", "route_C"]

SAT_IMAGE = Path(
    "/yh/study/sim_data/sim_competition_crop_check/"
    "sim_map_competition_roi_crop.png"
)
SAT_JSON = Path(
    "/yh/study/sim_data/sim_competition_crop_check/"
    "sim_map_competition_roi_crop_worldfile_epsg3826.json"
)

# Keep the retrieval embedding/output dimension identical across all backbones.
EMBED_DIM = 512

# Existing dataset preprocessing is kept unchanged.  For torchvision ImageNet
# backbones, the correct 224x224 resize + ImageNet normalization is applied
# inside visual_model_backbone.py immediately before the frozen backbone.
IMAGE_SIZE = 256
UAV_CENTER_CROP_SIZE = 256
UAV_CENTER_MAX_SQUARE_CROP = False
UAV_RESIZE_AFTER_CROP = None
TRAIN_UAV_AUGMENT = False

SAT_CROP_SIZE = 320
SAT_STRIDE = 32

USE_COORD_ENCODER = False
USE_QAH_MS_RELATION = False
USE_BASIN_RANK_MS = False
MOTION_SPATIAL_SIZE = 4

# Every backbone gets a fresh Route-A-only retrieval-head training.
# Do NOT reuse the MobileCLIP task-specific visual checkpoint across backbones.
VISUAL_EPOCHS = 30
VISUAL_LR = 3e-4
VISUAL_WEIGHT_DECAY = 1e-3
VISUAL_BATCH_SIZE = 64
VISUAL_CACHE_BATCH_SIZE = 64
VISUAL_EARLY_STOPPING_PATIENCE = 8
VISUAL_LABEL_SMOOTHING = 0.05
VISUAL_COORD_LOSS_WEIGHT = 0.25

MEANSHIFT_SCORE_TAU = 0.30
MEANSHIFT_BANDWIDTH_M = 8.0
MEANSHIFT_ITERATIONS = 3

# T2-only RTL-CRF.
GRID_SIZE = 6
LOCAL_PRIOR_JITTER_M = 12.0
CANDIDATE_CAPTURE_RADIUS_M = 7.5
MIN_TRAIN_CAPTURE_RATE = 0.95
WINDOW_STRIDE = 1
TOKEN_DIM = 192
TRANSITION_HIDDEN = 64
TEMPORAL_DROPOUT = 0.10

# No POSITION_SCALE_M.  All geometry stays in raw meter-based units.
EPOCHS = 40
LR = 2e-4
WEIGHT_DECAY = 1e-3
GRAD_CLIP_NORM = 5.0
TRAIN_BATCH_SIZE = 8
EVAL_BATCH_SIZE = 64
SEED = 2027
DEVICE = "cuda"
FEATURE_CACHE_DTYPE = "float16"
EARLY_STOPPING_PATIENCE = 12

LOSS_CRF = 1.0
LOSS_FINAL_COORD = 1.0
LOSS_PATH_COORD = 0.35

TRAIN_FRACTION = 0.70
VAL_FRACTION = 0.15
TEST_FRACTION = 0.15
SPLIT_GUARD_FRAMES = 5

JUMP_TOLERANCE_M = 3.0
VAL_RPE_WEIGHT = 0.25
VAL_JUMP_WEIGHT = 0.02
VAL_STATIONARY_WEIGHT = 0.05