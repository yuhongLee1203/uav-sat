...
ARCHITECTURE_NAME = os.environ.get(
    ...
    "FieldAnchorMobileNetV3SmallScheduledReferenceBCtoAForward4x6_v37",
)
...
if os.environ.get("UAVSAT_OUTPUT_DIR"):
    ...
    OUTPUT_DIR = PROJECT_ROOT / "outputs" / "bc_train_a_validation_4x6"
CHECKPOINT_DIR = OUTPUT_DIR / "checkpoints"
VISUAL_CHECKPOINT = CHECKPOINT_DIR / "visual_retrieval_train_BC_val_A.pt"
TEMPORAL_CHECKPOINT = CHECKPOINT_DIR / "scheduled_reference_train_BC_val_A.pt"
LATEST_TEMPORAL_CHECKPOINT = CHECKPOINT_DIR / "scheduled_reference_train_BC_val_A_latest.pt"
...
TRAIN_ROUTES = [row.strip() for row in os.environ.get("UAVSAT_TRAIN_ROUTES", "route_B,route_C").split(",") if row.strip()]
VALIDATION_ROUTES = [row.strip() for row in os.environ.get("UAVSAT_VALIDATION_ROUTES", "route_A").split(",") if row.strip()]
EVAL_ROUTES = [row.strip() for row in os.environ.get("UAVSAT_EVAL_ROUTES", "route_A").split(",") if row.strip()]
if not TRAIN_ROUTES or len(VALIDATION_ROUTES) != 1 or len(EVAL_ROUTES) != 1:
    raise ValueError("TRAIN_ROUTES needs >=1 route; validation/eval need exactly one route")
...
CONTROLLED_PROTOCOL_NAME = (
    "route_reference_motion_prior_forward4x6_MS_3frame_GRU_"
    "next_position_polynomial_Kalman"
    if ROUTE_REFERENCE_ONLY
    else (
        "scheduled_route_reference_forward4x6_MS_3frame_GRU_"
        "next_position_polynomial_Kalman"
        if SCHEDULED_ROUTE_REFERENCE
        else
        "frame_indexed_reference_forward4x6_MS_3frame_GRU_"
        "next_position_polynomial_Kalman"
        if FRAME_REFERENCE_SUPERVISION
        else "GT+smooth-jitter_controlled_forward4x6_MS_local_prior_3frame_"
...
# selected 4x6=24 forward candidates are encoded/scored by the visual model.
FORWARD_ONLY_LOCAL_SEARCH = os.environ.get("UAVSAT_EXPERIMENT_FORWARD_ONLY", "1") == "1"
FORWARD_SEARCH_ROWS = 4
