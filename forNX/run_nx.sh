...
RUN_ROOT="${PKG_ROOT}/runs/v39_nx_latency_${TS}"
...
[[ -s "${VISUAL_CKPT}" ]] || fail "missing V39 visual checkpoint: ${VISUAL_CKPT}; run scripts/prepare_v39_package.sh on the host first"
[[ -s "${TEMPORAL_CKPT}" ]] || fail "missing V39 temporal checkpoint: ${TEMPORAL_CKPT}; run scripts/prepare_v39_package.sh on the host first"
...
grep -q 'Weighted Centroid -> GRU -> Kalman -> ONE final MS -> Final' "${RUNTIME}/robust_tracker.py" || fail "V39 DirectFinalMS patch audit failed"
...
echo "================================================================================"
echo "V39 DIRECTFINALMS END-TO-END LATENCY BENCHMARK"
echo "Board   : ${BOARD_MODEL}"
echo "Runtime : mobilenet_v3_small + Weighted Centroid + 3-frame GRU + fixed-R Kalman + final MS 6x6/BW7"
...
RESULT="${RUN_ROOT}/v39_nx_latency_result.json"
...
for route in ("route_B", "route_C"):
    e = d.get(route, {}).get("EndToEndTiming", {})
    if not e or int(e.get("samples", 0)) <= 0:
        raise SystemExit(f"ERROR: missing EndToEndTiming for {route}")
    rows.append((route, e))

total = sum(int(e["samples"]) for _, e in rows)
mean_ms = sum(float(e["mean_ms"]) * int(e["samples"]) for _, e in rows) / total
fps = 1000.0 / mean_ms
payload = {
    "method": "V39 DirectFinalMS",
    "pipeline": "MobileNetV3-Small -> Weighted Centroid -> 3-frame GRU -> fixed-R Kalman -> final MeanShift 6x6 BW7 -> XY",
    ...
    "warmup_frames_per_route": int(rows[0][1].get("warmup_frames", 0)),
    "samples": total,
    "mean_ms": mean_ms,
    "fps": fps,
    "routes": {name: e for name, e in rows},
}
Path(result_path).write_text(json.dumps(payload, indent=2), encoding="utf-8")
print("\n================================================================================")
print("V39 NX LATENCY RESULT")
print(f"Device                  : {payload['cuda_device']}")
for name, e in rows:
    print(f"{name:23s}: mean={float(e['mean_ms']):.3f} ms | median={float(e['median_ms']):.3f} ms | P90={float(e['p90_ms']):.3f} ms | FPS={float(e['fps']):.2f}")
print("--------------------------------------------------------------------------------")
