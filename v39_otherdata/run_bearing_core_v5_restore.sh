...
if not all(checks.values()):
    raise SystemExit('ERROR: local bearing_iclr_ablation.py is not the completed Formal-V5 runner; do not overwrite it.')
PY
...
echo "Architecture unchanged; restored pre-Smooth-V1 V5 estimator dynamics."
...
- Restores pre-Smooth-V1 V5 estimator dynamics before retraining the temporal checkpoint.
- Reuses the existing visual checkpoint and retrains only temporal/estimator state on train_01.
- Core component table includes w/o GRU, w/o Kalman, w/o Final MeanShift, Full.
- Temporal table uses same restored 3-frame checkpoint with 1f/2f context truncation.
