...
# Old planned-route-gallery results must NEVER count as a cache hit.
base_ok(){ local m="$1" c="$2" d="$BASE_ROOT/$m/$c"; [[ -s "$d/result.json" && -s "$d/test_01_final_result.jpg" && -s "$d/test_02_final_result.jpg" ]] && grep -q '"uses_waypoint_or_route_prior": false' "$d/result.json" && grep -q '16x16=256' "$d/result.json"; }
...
if [[ ! -s "$WEIGHT_DIR/best_model.pth" || ! -s "$WEIGHT_DIR/training_configure.json" ]]; then
  ...
  URL='https://huggingface.co/HaoyZhou/bearinguav/resolve/main/Bearing_UAV.zip?download=true'
...
for ((b=0;b<${#pending[@]};b+=3)); do p=();n=();for s in 0 1 2;do i=$((b+s));((i<${#pending[@]}))||break;c="${pending[$i]}";run_off "$c" "${GPU_IDS[$s]}" & p+=("$!");n+=("$c");done;for i in "${!p[@]}";do wait "${p[$i]}"||{ echo "Bearing failed ${n[$i]}" >&2;exit 8;};done;done
...
University-1652/SUES-200/DenseUAV/GTA-UAV are native matching-to-tile runs:
  ...
  - positive satellite = the fixed 256px RST tile containing that observation
  - test gallery = ALL 256 fixed RST tiles in that city
  - NO waypoint, route centerline, temporal history, previous position, or local prior is fed to them
  - failure/drift/jumps/endpoint miss are left untouched
