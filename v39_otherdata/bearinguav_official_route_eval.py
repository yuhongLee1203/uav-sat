#!/usr/bin/env python3
"""Evaluate official Bearing-UAV VGG-16 on selected route frames.

Uses the authors' native four-neighbour RST input.  Bearing-UAV normalized
position coordinates are scaled by UNI_PIXEL=128 (not PATCH_SIZE=256).
"""
...
def _route_csv(dataset_root,prepared,city,route,city_rows,out):
    ...
    for frame in man:
        ...
        for col in ('p1_path','p2_path','p3_path','p4_path','target_path'): row[col]=_resolve(row[col],dataset_root)
        row['__frame_id']=int(frame['frame_id']); row['__gt_x_m']=float(frame['x_m']); row['__gt_y_m']=float(frame['y_m']); sel.append(row)
...
def _eval(model,dataset_class,csv_path,device,mpp,workers,batch_size):
    ...
    center_px=blocks*bearing.PATCH_SIZE+bearing.PATCH_SIZE
    pred_px=center_px+pp*UNI_PIXEL; gt_px=center_px+gp*UNI_PIXEL
    pred_m=pred_px*float(mpp); gt_m=gt_px*float(mpp); err=np.linalg.norm((pp-gp)*UNI_PIXEL*float(mpp),axis=1)
...
def main():
    ...
    for route in ('test_01','test_02'):
        csvp=out/f'{route}_selected_metadata.csv'; _route_csv(data,prepared,a.city,route,city_rows,csvp); met,pred_m,gt_m=_eval(model,dscls,csvp,device,mpp,a.workers,a.batch_size)
        man=_manifest(prepared/'routes'/route/'manifest.csv'); manifest_gt=np.asarray([[float(r['x_m']),float(r['y_m'])] for r in man],dtype=np.float64); mx=float(np.linalg.norm(manifest_gt-gt_m,axis=1).max())
        ...
        _figure(prepared,route,pred_m,gt_m,met,out/f'{route}_final_result.jpg','Bearing-UAV official VGG-16'); routes[route]=met; all_d+=met['distance_errors_m']; all_h+=met['heading_errors_deg']; rec_hits+=met['Recall@1_pct']*met['frames']/100
    ...
    payload={'method':'Bearing-UAV official VGG-16','source':'official liukejia121/bearinguav code + official pretrained cross_view checkpoint','city':a.city,'model_class':cls_name,'model_kwargs':model_kw,'metric_conversion':'official UNI_PIXEL=128','dataset_route_source':exp.get('test_route_source'),'evaluation_scope':'same selected test frames; native official four-neighbour RST pose regression','uses_v39_route_prior':False,'routes':routes,'aggregate_two_routes':agg}
