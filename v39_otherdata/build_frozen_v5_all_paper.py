...
PATCH_SIZE=256.0
MPP=0.25
ROUTE_MAP={"test_01":"nav50","test_02":"nav51"}
...
def recall_same_quadrant(rows,manifest,meta,origin):
    ...
    for r,m in zip(rows,manifest):
        ...
        px=(float(r['final_x'])+origin[0])/MPP-cx; py=(float(r['final_y'])+origin[1])/MPP-cy
...
def main():
    ...
    payload={'protocol':'Frozen V5 controlled_gt_jitter local-prior sequential evaluation','important_notes':{'Recall@1':'4-RST same-quadrant decision derived from continuous final position; not a native retrieval head.','Heading':'Current tracker heading is motion/route heading, not Bearing-UAV camera yaw; official HSR/MHE/MedHE remain N/A for Ours.','Navigation':'Current nav50/nav51 is offline route replay, not Bearing-Naver closed-loop; SR/SPL/NE replay diagnostics are kept separate.','Comparison':'MLE/MedLE/LSR formulas align, but protocol differs from Bearing-UAV four-RST pose regression.'},'overall':overall,'per_city':per_city,'literature':comparison}
