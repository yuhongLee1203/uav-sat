...
# The repository may already contain the V5 direct-delta2 source from a prior
...
helper = r'''def _calibrate_kalman_on_training_validation(args, config, tracker, visual, model, cache, route):
    ...
    blend_rows = []
    # Conservative residual Kalman fusion. B/C validation favored Kalman, but
    # held-out differences versus raw measurement were millimetric; cap the
    # prior contribution to avoid over-correcting strong visual measurements.
    for base in (0.00, 0.01, 0.02):
...
# Keep patience=4, promote output naming to V5, and use the minimal identifiable
# objective. The next-step loss trains the complete second-order branch without
# redundant velocity/acceleration pseudo-label losses.
python3 - <<'PY'
from pathlib import Path
...
s = s.replace('UAVSAT_LOSS_VELOCITY:-0.40', 'UAVSAT_LOSS_VELOCITY:-0.0')
s = s.replace('UAVSAT_LOSS_ACCELERATION:-0.25', 'UAVSAT_LOSS_ACCELERATION:-0.0')
p.write_text(s, encoding='utf-8')
print('[RUNNER V5] patience=4, minimal losses=measurement+next-step+variance-NLL: PASS')
PY
