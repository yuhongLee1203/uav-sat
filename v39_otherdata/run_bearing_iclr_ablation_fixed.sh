...
'''
new_delta2 = '''        # V5 third-frame-only direct second-order state.
        if frame_count >= 3:
...
if 'delta2_direct = torch.tanh(self.delta2_motion_head(accel_h))' not in s:
    if s.count(old_delta2) != 1:
        raise SystemExit(f'V5 patch failed: delta2 block matches={s.count(old_delta2)}')
...
new_accel = '''        if frame_count >= 3:
        ...
        acceleration = torch.cat([a_parallel, a_cross], dim=1)
'''
if 'delta2_direct[:, 0:1]' not in s:
    pos = s.find("new_motion = '''")
    hit = s.find(old_accel, pos)
    if pos < 0 or hit < 0:
        raise SystemExit('V5 patch failed: new_motion acceleration tail not found')
    s = s[:hit] + new_accel + s[hit + len(old_accel):]

if 'V5_DIRECT_NEXT_STEP_PATCH' not in s:
    boundary = "    s = s.replace(old_motion, new_motion, 1)\n\nold_init ="
    if boundary not in s:
        raise SystemExit('V5 patch failed: motion/init boundary not found')
...
if config_marker not in s:
    if s.count(config_needle) != 1:
        raise SystemExit(f'V5 config patch failed: delta2 config matches={s.count(config_needle)}')
...
print('[FORMAL V5] temporal patch source: PASS')
PY
...
new = '    # Legacy Context-GRU disabled; V5 owns the temporal model.\n'
if old in s:
    if s.count(old) != 1:
        raise SystemExit(f'legacy Context-GRU call count={s.count(old)}')
    s = s.replace(old, new, 1)
elif 'Legacy Context-GRU disabled' not in s:
    raise SystemExit('legacy Context-GRU insertion point not found')
...
helper = r'''def _calibrate_kalman_on_training_validation(args, config, tracker, visual, model, cache, route):
    ...
    def evaluate(stage, updates):
        apply(updates)
        r = tracker.evaluate_closed_loop(model, visual, cache, route, gt_state, val_range, device)
        row = snapshot()
        row.update({
            "stage": stage,
            "val_mle_m": float(r["mle"]),
            "val_p90_m": float(r["p90"]),
            "val_speed_mae": float(r["speed_mae"]),
            "val_progress_mae": float(r["progress_mae"]),
        })
        row["objective"] = float(
            row["val_mle_m"] + 0.08 * row["val_p90_m"]
            + 0.02 * row["val_speed_mae"] + 0.01 * row["val_progress_mae"]
        )
        profiles.append(row)
        return row
    def choose(rows):
    ...
    best = choose(rows + [step_best])

    selected = snapshot()
    mode = str(config.EXPERIMENT_KALMAN)
    config.EXPERIMENT_KALMAN = "none"
    no_k = tracker.evaluate_closed_loop(model, visual, cache, route, gt_state, val_range, device)
    config.EXPERIMENT_KALMAN = mode
    apply(selected)
    ...
    payload = {
        ...
        "validation_no_kalman_diagnostic": {
            "mle_m": float(no_k["mle"]), "p90_m": float(no_k["p90"])
        },
        "profiles": profiles,
...
compile(s, str(p), 'exec')
p.write_text(s, encoding='utf-8')
print('[FORMAL V5] runner + calibration patch: PASS')
PY
...
checks = {
    ...
    'train_only_calibration': 'formal_v5_direct_delta2_residual_kalman' in runner,
