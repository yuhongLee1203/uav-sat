...
# Paper experiments kept here:
#   ms1       : forward 3x6 [baseline], 4x6, 5x6, 6x6, 7x6
#   ms2       : centered 5x5, 6x6 [baseline], 7x7
#   meanshift : bandwidth 4, 8 [baseline], 12, 16 m
#
# Everything is EVAL ONLY and reuses the same FULL v8r1 checkpoint.
# Merge-radius and iteration-count sensitivity are intentionally not tested.

GROUP="${1:-all}"
PYTHON_BIN="${PYTHON_BIN:-python3}"
CPU_THREADS="${UAVSAT_CPU_THREADS:-1}"
LATENCY_WARMUP_FRAMES="${UAVSAT_LATENCY_WARMUP_FRAMES:-10}"
...
echo "METHOD sensitivity + latency: STRICTLY EVAL ONLY; CPU threads=${CPU_THREADS}" >&2
...
run_case() {
  ...
  echo "  fixed                   = same FULL weights, KF, merge radius=2m, iterations=3"
  echo "  latency warm-up         = ${LATENCY_WARMUP_FRAMES} frames/route"
  echo "================================================================================================"
  ...
  UAVSAT_LATENCY_WARMUP_FRAMES="${LATENCY_WARMUP_FRAMES}" \
  "${PYTHON_BIN}" - <<'PY'
import csv
import math
import os
...
warmup_frames = max(0, int(os.environ.get("UAVSAT_LATENCY_WARMUP_FRAMES", "10")))
...
bandwidth_m = float(os.environ["UAVSAT_MS_BANDWIDTH_M"])
merge_radius = float(os.environ["UAVSAT_METHOD_MS_MERGE_RADIUS_M"])
iterations = int(os.environ["UAVSAT_MS_ITERATIONS"])
...
if config.GRU_ABLATION != "full":
    raise SystemExit("ERROR: method sensitivity must keep FULL GRU")
if ms1_rows not in {3, 4, 5, 6, 7}:
    raise SystemExit("ERROR: MS1 rows must be 3,4,5,6,7")
if ms2_grid not in {5, 6, 7}:
    raise SystemExit("ERROR: MS2 grid must be 5,6,7")

# Capture the shared FULL checkpoint/cache locations before isolating outputs.
...
best_checkpoint = shared_checkpoint_dir / f"reference_prior_compact_gru_A_native_v8r1_full_{config.BACKBONE_KEY}.pt"
latest_checkpoint = shared_checkpoint_dir / f"reference_prior_compact_gru_A_native_v8r1_full_{config.BACKBONE_KEY}_latest.pt"
if best_checkpoint.exists():
    ...
    raise SystemExit("ERROR: FULL v8r1 checkpoint not found; GPU6 is eval-only")

# Change only the requested inference-time spatial variable.
...
config.MEANSHIFT_BANDWIDTH_M = bandwidth_m
config.MEANSHIFT_MODE_MERGE_RADIUS_M = merge_radius
config.MEANSHIFT_ITERATIONS = iterations
...
destination_checkpoint = case_checkpoint_dir / f"reference_prior_compact_gru_A_native_v8r1_full_{config.BACKBONE_KEY}.pt"
...
# Import only after config paths and method parameters are fixed.
...
def _p90(values):
    a = np.asarray(values, dtype=np.float64)
    return float(np.percentile(a, 90.0)) if a.size else float("nan")


def _lsr15(values):
    a = np.asarray(values, dtype=np.float64)
    return float(np.mean(a <= 15.0) * 100.0) if a.size else float("nan")


def _select_forward_positions(heading_rad, rows, grid_size, batch, device):
    """Return flattened row-major positions for exact forward N x 6.

    IMPORTANT: regular_grid_indices() is generated row-major from an axis-aligned
    lattice (offset_y outer loop, offset_x inner loop). We use those integer
    lattice positions directly. No metric-coordinate equality/tolerance is used,
    so oblique heading and tiny XY distortions cannot split a real row.

    For G=2N, positive direction uses offsets 0..N-1; negative direction uses
    -1..-N. The lateral width is always the six offsets nearest the geometric
    centre: -3,-2,-1,0,1,2. For N=3 this exactly reproduces the original 3x6.
    """
    g = int(grid_size)
    if g < 6 or g < 2 * int(rows) or g % 2 != 0:
        raise RuntimeError(f"invalid source grid {g} for forward {rows}x6")

    headings = torch.as_tensor(heading_rad, dtype=torch.float32, device=device).reshape(-1)
    ...
    lateral = list(range(g // 2 - 3, g // 2 + 3))
    selected_batches = []
    for b in range(batch):
        c = float(torch.cos(headings[b]).item())
        s = float(torch.sin(headings[b]).item())
        use_x = abs(c) >= abs(s)
        positive = (c >= 0.0) if use_x else (s >= 0.0)

        if positive:
            forward = list(range(g // 2, g // 2 + rows))
        else:
            forward = list(range(g // 2 - 1, g // 2 - 1 - rows, -1))

        flat = []
        if use_x:
            # forward axis = x (column), lateral axis = y (row)
            for x in forward:
                for y in lateral:
                    flat.append(y * g + x)
        else:
            # forward axis = y (row), lateral axis = x (column)
            for y in forward:
                for x in lateral:
                    flat.append(y * g + x)

        if len(flat) != rows * 6 or min(flat) < 0 or max(flat) >= g * g:
            raise RuntimeError("forward lattice index construction failed")
        selected_batches.append(torch.tensor(flat, dtype=torch.long, device=device))
...
def _run_meanshift_with_timing(logits, centers):
    ...
    for b in range(int(mode_weights.shape[0])):
        ...
        _, reduction_ms = _gpu_reduction_time_ms(
            lambda aw=active_weights, am=active_modes: (aw[:, None] * am).sum(dim=0)
        )
        final_times.append(float(reduction_ms))

    return soft_xy, soft_support, mode_weights, {
        "total_ms": float(total_ms),
        "final_active_average_ms": float(np.mean(final_times)),
...
def _method_ms1_search(visual, uav_clip, prior_xy, heading_rad, device):
    ...
    full_indices = train.rt.regular_grid_indices(
        ...
        center, source_grid, config.SAT_STRIDE, visual.device,
    )
    selected = _select_forward_positions(
        heading_rad, ms1_rows, source_grid, int(full_indices.shape[0]), full_indices.device
    )
    batch_idx = torch.arange(full_indices.shape[0], device=full_indices.device)[:, None]
    ...
    z_sat = visual.model.encode_sat_from_clip(
        satellite_clip.reshape(-1, satellite_clip.shape[-1]), centers.reshape(-1, 2)
    ...
    raw_top1_xy = centers[torch.arange(centers.shape[0], device=centers.device), top1_idx]
...
def _raw_candidate_batch(visual, uav_clip, center_xy, grid_size):
    indices = train.rt.regular_grid_indices(
        visual.gallery["xy"], visual.gallery["pixel"], visual.pixel_index,
        center_xy, int(grid_size), config.SAT_STRIDE, visual.device,
    )
    centers = visual.gallery["xy"][indices]
    satellite_clip = visual.gallery["clip_feat"][indices]
    z_uav = visual.model.encode_uav_from_clip(uav_clip)
    ...
    raw_top1_xy = centers[torch.arange(centers.shape[0], device=visual.device), raw_index]
    ...
    zeros_count = torch.zeros(centers.shape[0], dtype=torch.long, device=centers.device)
...
def _timed_ms2_center_search(visual, uav_clip, kalman_xy, device):
    ...
    batch = _raw_candidate_batch(
        visual, uav_clip, train.rt._tensor_xy(kalman_xy, device), int(config.MS2_GRID_SIZE)
    )
...
def _timed_kalman_conditioned_ms2(visual, uav_clip, kalman_xy, device):
    batch = _timed_ms2_center_search(visual, uav_clip, kalman_xy, device)
    ...
    distance_squared = (batch.centers - kalman_center[:, None, :]).square().sum(dim=2)
    posterior_logits = batch.raw_logits - 0.5 * prior_weight * distance_squared / (sigma * sigma)
    posterior_prob = torch.softmax(posterior_logits / float(config.MEANSHIFT_SCORE_TAU), dim=1)
    ...
    weighted_all_xy, weighted_all_ms = _gpu_reduction_time_ms(
    ...
    result.final_active_average_latency_ms = float(ms_timing["final_active_average_ms"])
...
# Install the two method variants inside this eval-only process.
...
def _timed_forward_frame(model, visual, uav_clip, state, device):
    ...
    ms1 = frame["ms1"]
    ms2 = frame["ms2"]
    if reference_index >= warmup_frames:
        ...
        _append(route_name, "ms2_final_active_average_latency_ms", ms2.final_active_average_latency_ms)
        _append(route_name, "ms2_all_candidate_average_latency_ms", ms2.all_candidate_average_latency_ms)
...
print("MS1 SUPPORT:", f"{ms1_rows}x6", "candidates=", ms1_rows * 6, flush=True)
print("MS2 GRID:", f"{ms2_grid}x{ms2_grid}", flush=True)
print("MEANSHIFT BANDWIDTH:", bandwidth_m, "m", flush=True)
print("MODE: eval only + synchronized latency", flush=True)

sys.argv = ["train_multirate_a.py", "--mode", "eval"]
train.main()

# Accuracy uses all B/C frames. Latency excludes only the warm-up frames above.
summary_rows = []
for route_name in ("route_C", "route_B"):
    ...
    summary_rows.append({
        ...
        "MeanShift_Bandwidth_m": bandwidth_m,
        ...
        "MS1_MeanShift_Latency_ms": _mean(stats.get("ms1_meanshift_latency_ms", [])),
        "MS2_CenterSearch_Latency_ms": _mean(stats.get("ms2_center_search_latency_ms", [])),
        "MS2_MeanShift_Total_Latency_ms": _mean(stats.get("ms2_meanshift_latency_ms", [])),
        "MS2_FinalActiveAverage_Latency_ms": _mean(stats.get("ms2_final_active_average_latency_ms", [])),
        "MS2_AllCandidateAverage_Latency_ms": _mean(stats.get("ms2_all_candidate_average_latency_ms", [])),
        "MS2_ActiveModes_Avg": _mean(stats.get("ms2_active_modes", [])),
    })

if len(summary_rows) == 2:
    avg = {"Case": tag, "Route": "B+C average"}
    text_keys = {"Case", "Route", "MS1_Support", "MS2_Grid"}
    for key in summary_rows[0]:
        if key not in text_keys:
            avg[key] = (float(summary_rows[0][key]) + float(summary_rows[1][key])) / 2.0
    avg["MS1_Support"] = f"{ms1_rows}x6"
    avg["MS2_Grid"] = f"{ms2_grid}x{ms2_grid}"
...
if summary_rows:
    ...
    for row in summary_rows:
        print(
            ...
            "E2E=%.3fms MS1=%.3fms CenterMS=%.3fms MS2MeanShift=%.3fms "
            "FinalActiveAvg=%.6fms AllCandidateAvg=%.6fms ActiveModes=%.2f"
            % (
                row["Route"], row["MeanShift_MLE_m"], row["MeanShift_P90_m"],
                row["MeanShift_LSR15_pct"], row["WeightedAll_MLE_m"],
                row["WeightedAll_P90_m"], row["WeightedAll_LSR15_pct"],
                row["E2E_Latency_ms"], row["MS1_Latency_ms"],
                row["MS2_CenterSearch_Latency_ms"], row["MS2_MeanShift_Total_Latency_ms"],
                row["MS2_FinalActiveAverage_Latency_ms"],
                row["MS2_AllCandidateAverage_Latency_ms"], row["MS2_ActiveModes_Avg"],
            )
        )
