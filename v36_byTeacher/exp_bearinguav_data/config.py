"""Original v36_byTeacher configuration with data/output paths overridden only."""
...
# Same checkpoint naming and training settings; only keep artifacts in this experiment.
...
VISUAL_CHECKPOINT = CHECKPOINT_DIR / ("visual_retrieval_3train_1val_" + BACKBONE_KEY + ".pt")
TEMPORAL_CHECKPOINT = CHECKPOINT_DIR / (
    ...
    + str(TEMPORAL_EXTRA_A_STRIDE) + "_v7.pt"
)
LATEST_TEMPORAL_CHECKPOINT = CHECKPOINT_DIR / (
    "multiroute3train1val_referenceprior_forward3x6_ms_previous_position_"
    + str(EXPERIMENT_FRAME_COUNT) + "frame_" + BACKBONE_KEY
    + "_direct_softms_gru_motion_heading_learned_variance_multirate_A_native_plus_stride"
    + str(TEMPORAL_EXTRA_A_STRIDE) + "_v7_latest.pt"
)
