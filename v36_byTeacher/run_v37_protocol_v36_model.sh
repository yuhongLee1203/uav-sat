...
echo "[2/2] run exact v37 non-model protocol with only the v36 model module substituted"
...
# Load v37 first. Everything outside the learned model comes from v37:
# data split, scheduled references, direct reference XY, forward 4x6 geometry,
# visual training, losses, optimizer/TBPTT schedule, motion-state stabilization,
# constrained external Kalman, validation and evaluation.
sys.path.insert(0, str(v37_dir))
import config

# Only model-specific dimensions/semantics are changed back to v36_byTeacher.
config.ARCHITECTURE_NAME = (
    "V36_byTeacher_2FramePureModel_on_v37_"
...
# Inject v36_byTeacher/visual_model.py under the module name expected by v37.
# v37 visual_localizer/robust_tracker are imported after this, therefore their
# data/reference/search/training/filter implementation remains exactly v37.
...
print("=== CONTROL: v37 protocol / v36 model only ===", flush=True)
print("model module:", v36_dir / "visual_model.py", flush=True)
print("v36 frame count:", config.EXPERIMENT_FRAME_COUNT, flush=True)
print("v36 manual model constraints:", config.MANUAL_DYNAMICS_CONSTRAINTS, flush=True)
