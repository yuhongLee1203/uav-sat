...
mkdir -p logs/delayed_pair_route_tube_recovery_center6x6
...
print('[preflight] route-tube recovery: no B/C per-frame reference prior')
PY

echo "[preflight] centered 6x6 MeanShift = 36 patches"
echo "[preflight] soft/hard route corridor = ${SOFT_CORRIDOR}m / ${HARD_CORRIDOR}m"
echo "[preflight] max causal route progress per frame = ${MAX_PROGRESS}m"
...
run_one () {
  local arch="$1"
  echo "[GPU 6] starting route-tube recovery ${arch}"
  ...
  "$PYTHON_BIN" gpu_grid_runner.py delayed_pair_route_tube_recovery_experiment.py \
    --mode train-eval \
    --arch "${arch}" \
    --device cuda:0 \
    --epochs "${EPOCHS}" \
    ...
    2>&1 | tee "logs/delayed_pair_route_tube_recovery_center6x6/${arch}_gpu6.log"
}

run_one KG
run_one GK

echo "Route-tube recovery KG/GK experiments finished."
