...
# v39_DirectFinalMS first removes the legacy KF2/MS2 tail and builds the
# selected single final-MS path.  That patch historically also changed the
# Forward-3x6 visual decoder from SoftMS to Weighted Centroid.  The current
# selected method restores the original Forward-3x6 SoftMS decoder while
# keeping the cleaned GRU -> fixed Kalman -> one final 6x6 MS tail.
...
# The source contains three soft_mean_shift call sites after restoration:
#   1) Forward-3x6 decoder,
#   2) SoftMS mode recomputation used by the existing uncertainty calculation,
#   3) final 6x6 refinement after Kalman.
if s.count("soft_mean_shift(") != 3:
    raise SystemExit(
        f"ERROR: expected 3 SoftMS call sites after front restoration, got {s.count('soft_mean_shift(')}"
    )
