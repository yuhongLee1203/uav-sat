...
SUITE_ROOT="${SOFTMS_ABLATION_DIR:-${ROOT}/softms_5x5_core_ablation_${TS}}"
...
{
  "method": "5x5 geometry -> forward 15 -> front SoftMS -> temporal GRU -> constant velocity -> fixed-R Kalman -> final 5x5 SoftMS",
  "git_sha": "${GIT_SHA}",
  "front_grid": 5,
  "front_candidates": 15,
  "final_ms_grid": 5,
...
make_runtime() {
  ...
  grep -q 'forward-15 local posterior' "${runtime}/robust_tracker.py" || fail "front 5x5/15 audit failed"
}
...
run_cfg() {
  ...
  echo "================================================================================"
  echo "[RUN] ${name} | frames=${frames} | front=5x5->15 | gru=$((1-disable_gru)) | kalman=${kalman} | finalMS=${ms_enabled} grid=${grid}"
  echo "================================================================================"
  (
    cd "${runtime}"
...
p=out/'robust_tracker_summary.json'; d=json.loads(p.read_text(encoding='utf-8'))
d.update({
 'experiment_tag':name,'experiment_category':category,'experiment_anchor':'softms',
 'experiment_frame_count':frames,'experiment_forward_only':True,
 'front_grid_size':5,'front_candidate_count':15,
 'experiment_disable_gru':disable,'experiment_kalman':kalman,
 'ms_enabled':ms,'ms_grid_size':grid,'checkpoint_retrained':category=='temporal_frames',
})
for route in ('route_B','route_C'):
    if route in d:
        dec=d[route].get('VisualObservationDecoder')
...
# 1/2/3-frame temporal study: each variant receives fresh Route-A training.
...
# Core module ablation. Forward 5x5->15 is fixed in every row.
...
# Final-MS sensitivity only; front remains 5x5->15 for every row.
...
names=['full_model','abl_no_gru','abl_no_kalman','abl_no_final_ms',
       'temporal_1frame','temporal_2frame','temporal_3frame',
       'ms_window_4x4','ms_window_5x5','ms_window_6x6','ms_window_7x7','ms_window_8x8']

def metric(a):
    a=np.asarray(a,float)
    return {'frames':int(len(a)),'MLE_m':float(a.mean()),'MedLE_m':float(np.median(a)),
...
def errors(out,route):
    ...
    if len(files)!=1: raise SystemExit(f'{out}/{route}: expected one CSV, got {len(files)}')
    with files[0].open(newline='',encoding='utf-8') as f:
        return np.asarray([float(r['error_final_m']) for r in csv.DictReader(f)])

rows=[]; payload={}
for name in names:
    out=suite/name; d=json.loads((out/'robust_tracker_summary.json').read_text())
    if int(d.get('front_grid_size',0))!=5 or int(d.get('front_candidate_count',0))!=15:
        raise SystemExit(f'AUDIT FAILED: {name} is not front 5x5/15')
    eb,ec=errors(out,'route_B'),errors(out,'route_C'); ea=np.concatenate([eb,ec])
    mb,mc,ma=metric(eb),metric(ec),metric(ea); rb,rc=d['route_B'],d['route_C']
    warm=int(rb.get('MS_LatencyWarmupFrames',30)); nb=max(len(eb)-warm,1); nc=max(len(ec)-warm,1)
    ...
    row={'Experiment':name,'Category':d.get('experiment_category'),'Front':'5x5->15 SoftMS','Frames':d.get('experiment_frame_count'),
         'GRU':'no' if d.get('experiment_disable_gru') else 'yes','Kalman':d.get('experiment_kalman'),
         'FinalMS':'yes' if d.get('ms_enabled') else 'no','MS_grid':d.get('ms_grid_size'),
         'B_MLE_m':mb['MLE_m'],'C_MLE_m':mc['MLE_m'],'BC_MLE_m':ma['MLE_m'],'BC_MedLE_m':ma['MedLE_m'],
         'BC_P90_m':ma['P90_m'],'BC_P95_m':ma['P95_m'],'BC_P99_m':ma['P99_m'],
         'BC_LSR5_pct':ma['LSR5_pct'],'BC_LSR10_pct':ma['LSR10_pct'],'BC_LSR15_pct':ma['LSR15_pct'],'BC_LSR20_pct':ma['LSR20_pct'],
         'PureFinalMSLatency_ms':ms,'PureFinalMS_FPS':1000/ms if ms>0 else 0.0,
         'E2E_mean_ms':e2,'E2E_FPS':1000/e2 if e2>0 else 0.0}
    rows.append(row); payload[name]={'route_B':mb,'route_C':mc,'combined':ma,'metadata':{k:d.get(k) for k in ['experiment_category','experiment_frame_count','experiment_disable_gru','experiment_kalman','ms_enabled','ms_grid_size','front_grid_size','front_candidate_count']}}
...
by={r['Experiment']:r for r in rows}; fmt=lambda x,n=3:f'{float(x):.{n}f}'
md=['# V39 5x5/15 SoftMS Ablation Results','',
...
for n,label in [('full_model','Full'),('abl_no_gru','w/o GRU'),('abl_no_kalman','w/o Kalman'),('abl_no_final_ms','w/o Final MS')]:
    r=by[n]; md.append(f"| {label} | {r['GRU']} | {r['Kalman']} | {r['FinalMS']} | {fmt(r['B_MLE_m'])} | {fmt(r['C_MLE_m'])} | {fmt(r['BC_MLE_m'])} | {fmt(r['BC_P90_m'])} | {fmt(r['BC_LSR5_pct'],2)}% |")
md += ['','## Table 2. Temporal frames','','| Frames | B MLE | C MLE | B+C MLE | B+C P90 | B+C LSR@5 |','|---:|---:|---:|---:|---:|---:|']
...
md += ['','## Table 3. Final MeanShift window','','| Window | Candidates | B MLE | C MLE | B+C MLE | B+C P90 | B+C LSR@5 | Pure final-MS latency | FPS |','|---|---:|---:|---:|---:|---:|---:|---:|---:|']
for g in range(4,9):
...
r=by['full_model']; md += ['','## Full-model E2E runtime','',f"- Mean: **{fmt(r['E2E_mean_ms'])} ms/frame**",f"- FPS: **{fmt(r['E2E_FPS'],2)}**"]
(suite/'paper_tables.md').write_text('\n'.join(md)+'\n',encoding='utf-8')
(suite/'audit_report.json').write_text(json.dumps({'status':'PASS','front_grid':5,'front_candidates':15,'default_final_ms_grid':5,'experiments':names,'raw_data_saved':True},indent=2),encoding='utf-8')
print('\n'+'='*96); print('5x5/15 SOFTMS ABLATION COMPLETE'); print('Results:',suite); print('Tables :',suite/'paper_tables.md'); print('='*96)
PY
