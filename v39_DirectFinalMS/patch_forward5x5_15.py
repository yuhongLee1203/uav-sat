...
# Keep the visual retrieval checkpoint/training geometry untouched.  Only the
# online local acquisition window changes from 6x6/18 scored candidates to a
# 5x5 geometry with the heading-forward 3x5=15 candidates scored.
...
new_guard = '''def forward_3x6_candidate_batch(visual, uav_clip, center_xy, heading_rad, grid_size=5):
    ...
    causal heading.  Exactly the top 3x5=15 candidates are then encoded/scored;
    rear candidates are excluded from UAV-SAT similarity and front SoftMS.
    The legacy function name is retained for checkpoint/source compatibility.
    """
    grid_size = int(grid_size)
...
tracker = tracker.replace(
    ...
    'summary["VisualObservationDecoder"] = "forward 15-of-5x5 soft mean shift"',
)
tracker = tracker.replace(
    ...
    'Forward 15-of-5x5 Soft MeanShift visual observation -> GRU -> robust constrained route-coordinate Kalman -> one final 5x5 MeanShift -> final XY.',
)
tracker = tracker.replace(
    'Forward 3x6 SoftMS -> GRU -> Kalman -> ONE final 6x6 MS -> Final',
    'Forward 15-of-5x5 SoftMS -> GRU -> Kalman -> ONE final 5x5 MS -> Final',
)
tracker = tracker.replace(
    'Forward-3x6 decoder',
    'Forward-15-of-5x5 decoder',
)
tracker = tracker.replace(
    'Forward 3x6 candidates',
    'Forward 15-of-5x5 candidates',
)

# Cosmetic/documentation strings in the base source.  These do not affect
# checkpoint names, which intentionally remain backward-compatible.
tracker = tracker.replace(
    'Confidence from the actual 6x6 local posterior, never hypothesis count.',
    'Confidence from the actual forward-15 local posterior, never hypothesis count.',
)
tracker = tracker.replace(
    'compare the best raw\n    # UAV-SAT similarity of every 3x6 window.',
...
print("[OK] V39 online search: 5x5 geometry -> heading-forward 15 -> SoftMS")
