#!/usr/bin/env python3
"""Plot original v39 DirectFinalMS Route-B/C final trajectories.

This diagnostic script reads the per-frame inference CSVs already written by
robust_tracker.py and projects metric XY coordinates back onto the original
satellite image using the same map geometry as the v39 data code.

Only two trajectories are drawn:
  - GT/reference trajectory: green
  - final DirectFinalMS prediction: red

No training or inference behavior is changed.
"""
...
def _metric_to_latlon(
    ...
    lon = float(origin_lon) + math.degrees(
        float(x_m) / (EARTH_RADIUS_M * cos_lat)
    )
    return lat, lon
...
def _crop_bounds(
    point_sets: Iterable[Sequence[Point]],
    width: int,
    height: int,
    margin: int = 180,
...
def _find_csv(
    ...
    if not matches:
        raise FileNotFoundError(
            f"No {route_name} inference CSV found under {output_dir}"
        )
    return matches[-1]
...
def render_route(
    ...
    if not reference or not final:
        raise RuntimeError(
            f"CSV lacks usable GT/final coordinates: {csv_path}"
        )
    ...
    # Draw only the two trajectories requested for clean comparison.
    _draw_line(
        draw,
        reference,
        (40, 255, 100, 245),
    ...
    # Start/end markers only; no intermediate points or error connectors.
    _draw_marker(
        draw,
        reference[0],
        (40, 255, 100, 255),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "output_wc_single"),
    )
    parser.add_argument(
        "--data-root",
    ...
    parser.add_argument(
        "--visual-checkpoint",
        default=str(DEFAULT_VISUAL_CKPT),
    )
    parser.add_argument(
        "--routes",
        nargs="+",
...
def main() -> None:
    ...
    if not summary_path.exists():
        raise FileNotFoundError(
            f"Missing inference summary: {summary_path}"
        )
    if not visual_checkpoint.exists():
        raise FileNotFoundError(
            f"Missing visual checkpoint: {visual_checkpoint}"
        )
    ...
    for route_name in args.routes:
        ...
        if not isinstance(route_summary, dict):
            raise KeyError(
                f"Summary has no route result: {route_name}"
            )
        render_route(
            route_name,
            output_dir,
