...
if [[ "${RUN_ALL_EXPERIMENTS:-0}" == "1" ]]; then
  ...
  run_one() {
    ...
    echo "[START][GPU ${gpu}] ${name}: jitter=${jitter}, kf_w=${kf_weight}, ref_w=${ref_weight}, bw=${bandwidth}, ms2_ref_noise=${ref_noise}"
    ...
    MS2_REFERENCE_PERTURB_M="${ref_noise}" \
    EXPERIMENT_TAG="${name}" \
  ...
  # Main selected method first; also safely warms reusable image/backbone features.
  run_one 0 "main_full_j8" 8 1.50 2.50 5.0 0.0

  # A) MS2 component ablation + one bandwidth sensitivity point.
  (
    run_one 0 "abl_visual_only"       8 0.00 0.00 5.0 0.0
    run_one 0 "abl_visual_kf"         8 1.50 0.00 5.0 0.0
    run_one 0 "abl_visual_reference"  8 0.00 2.50 5.0 0.0
    run_one 0 "sens_bandwidth_4"      8 1.50 2.50 4.0 0.0
  ) & pid0=$!

  # B) MS2-reference robustness. JITTER_M remains fixed at the main setting;
  # only the reference prior entering MS2 receives deterministic Gaussian noise.
  (
    run_one 5 "robust_ms2ref_noise_4"   8 1.50 2.50 5.0 4.0
    run_one 5 "robust_ms2ref_noise_8"   8 1.50 2.50 5.0 8.0
    run_one 5 "robust_ms2ref_noise_12"  8 1.50 2.50 5.0 12.0
    run_one 5 "robust_ms2ref_noise_16"  8 1.50 2.50 5.0 16.0
    run_one 5 "sens_bandwidth_6"        8 1.50 2.50 6.0 0.0
  ) & pid5=$!

  # C) Prior-weight sensitivity around the selected defaults.
  (
    run_one 6 "sens_reference_w1p5"   8 1.50 1.50 5.0 0.0
    run_one 6 "sens_reference_w3p5"   8 1.50 3.50 5.0 0.0
    run_one 6 "sens_kalman_w0p75"     8 0.75 2.50 5.0 0.0
    run_one 6 "sens_kalman_w2p25"     8 2.25 2.50 5.0 0.0
  ) & pid6=$!
...
def metric_row(name, d, kind="v39"):
    ...
    return {
        ...
        "MS2RefNoiseSigma_m": d.get("ms2_reference_perturb_m", "-"),
...
columns = [
    "Experiment", "Type", "Jitter_m", "MS2RefNoiseSigma_m", "KF_weight", "Reference_weight", "Bandwidth_m",
...
with md_path.open("w", encoding="utf-8") as f:
    ...
    f.write("| Experiment | Jitter | MS2 Ref Noise | KF w | Ref w | BW | B MLE | C MLE | B+C MLE | Delta vs Main | B LSR@5 | C LSR@5 | B Jump | C Jump |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for r in rows:
        def fmt(v, n=3):
            if isinstance(v, str):
        ...
        f.write(
            f"| {r['Experiment']} | {fmt(r['Jitter_m'],1)} | {fmt(r['MS2RefNoiseSigma_m'],1)} | "
            f"{fmt(r['KF_weight'],2)} | {fmt(r['Reference_weight'],2)} | {fmt(r['Bandwidth_m'],1)} | "
...
export MS2_REFERENCE_PERTURB_M="${MS2_REFERENCE_PERTURB_M:-0.0}"
...
d["ms2_reference_perturb_m"] = float(os.environ.get("MS2_REFERENCE_PERTURB_M", "0.0"))
