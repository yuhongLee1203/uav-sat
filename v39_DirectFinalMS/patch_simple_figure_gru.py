...
if "self.gru = nn.GRUCell(feature_dim * 6, hidden_dim)" not in s:
    if s.count("self.gru = nn.GRUCell(feature_dim * 5, hidden_dim)") == 1:
        s = s.replace("self.gru = nn.GRUCell(feature_dim * 5, hidden_dim)", "self.gru = nn.GRUCell(feature_dim * 6, hidden_dim)", 1)
    elif s.count("self.gru = nn.GRUCell(feature_dim * 4, hidden_dim)") == 1:
        s = s.replace("self.gru = nn.GRUCell(feature_dim * 4, hidden_dim)", "self.gru = nn.GRUCell(feature_dim * 6, hidden_dim)", 1)
    else:
...
sub1(r'^LOSS_MEASUREMENT\s*=\s*[0-9.]+\s*$', 'LOSS_MEASUREMENT = float(os.environ.get("UAVSAT_LOSS_MEASUREMENT", "2.50"))', 'measurement loss')
sub1(r'^LOSS_NEXT_STEP\s*=\s*[0-9.]+\s*$', 'LOSS_NEXT_STEP = float(os.environ.get("UAVSAT_LOSS_NEXT_STEP", "1.50"))', 'next-step loss')
sub1(r'^LOSS_VELOCITY\s*=\s*[0-9.]+\s*$', 'LOSS_VELOCITY = float(os.environ.get("UAVSAT_LOSS_VELOCITY", "0.10"))', 'velocity loss')
sub1(r'^TEMPORAL_LR\s*=\s*[0-9.eE+-]+\s*$', 'TEMPORAL_LR = float(os.environ.get("UAVSAT_TEMPORAL_LR", "2e-4"))', 'temporal lr')
sub1(r'^RNN_DROPOUT\s*=\s*[0-9.]+\s*$', 'RNN_DROPOUT = float(os.environ.get("UAVSAT_RNN_DROPOUT", "0.10"))', 'gru dropout')
sub1(r'^MOTION_VELOCITY_EMA_ALPHA\s*=\s*[0-9.]+\s*$', 'MOTION_VELOCITY_EMA_ALPHA = float(os.environ.get("UAVSAT_MOTION_VEL_ALPHA", "0.35"))', 'motion velocity alpha')
sub1(r'^MOTION_POLYNOMIAL_STEP_EMA_ALPHA\s*=\s*[0-9.]+\s*$', 'MOTION_POLYNOMIAL_STEP_EMA_ALPHA = float(os.environ.get("UAVSAT_MOTION_STEP_ALPHA", "0.40"))', 'motion step alpha')
sub1(r'^EARLY_SCORE_SPEED_WEIGHT\s*=\s*[0-9.]+\s*$', 'EARLY_SCORE_SPEED_WEIGHT = float(os.environ.get("UAVSAT_EARLY_SPEED_WEIGHT", "0.20"))', 'early speed weight')
...
sub1(r'^EARLY_SCORE_HEADING_WEIGHT\s*=\s*[0-9.]+\s*$', 'EARLY_SCORE_HEADING_WEIGHT = float(os.environ.get("UAVSAT_EARLY_HEADING_WEIGHT", "0.01"))', 'early heading weight')
...
sub1(r'^EARLY_STOP_MIN_DELTA\s*=\s*[0-9.]+\s*$', 'EARLY_STOP_MIN_DELTA = float(os.environ.get("UAVSAT_EARLY_MIN_DELTA", "0.01"))', 'early stop delta')
